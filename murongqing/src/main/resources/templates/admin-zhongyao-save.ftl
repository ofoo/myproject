...
<div class="container">
    <form class="form-horizontal" id="dataForm">
        ...
        <div class="form-group">
            <label for="sketch" class="col-md-2 control-label text-danger">类别</label>
            <div class="col-md-5">
                <select class="form-control" name="categoryId">
                    <option value="0">类别</option>
                    <#list categoryList as category>
                    <option value="${category.id}" <#if ((zhongyao.categoryId)!0)==category.id>selected</#if>>${category.name}</option>
                    </#list>
                </select>
            </div>
        </div>
